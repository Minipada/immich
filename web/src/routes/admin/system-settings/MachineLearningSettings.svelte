<script lang="ts">
  ...
  // Sentinel for "detect automatically", since the underlying config field is `string | null` and a
  // <select> element cannot bind an option to null.
  const AUTO_LANGUAGE = 'auto';

  // Every language faster-whisper recognizes, so an admin can only ever choose a code the model
  // actually supports. Sourced from faster_whisper.tokenizer._LANGUAGE_CODES.
  const TRANSCRIPTION_LANGUAGES: { code: string; name: string }[] = [
  ...
  const languageOptions = $derived([
    { value: AUTO_LANGUAGE, text: $t('admin.machine_learning_transcription_language_auto') },
    ...TRANSCRIPTION_LANGUAGES.map(({ code, name }) => ({ value: code, text: name })),
  ]);
</script>
  ...
  <div in:fade={{ duration: 500 }}>
    <form autocomplete="off" class="mx-4 mt-4" onsubmit={(event) => event.preventDefault()}>
      ...
      <SettingAccordion
        ...
        subtitle={$t('admin.machine_learning_transcription_description')}
      >
        <div class="mt-4 ml-4 flex flex-col gap-4">
          <SettingSwitch
            title={$t('admin.machine_learning_transcription_enabled')}
            subtitle={$t('admin.machine_learning_transcription_enabled_description')}
            bind:checked={configToEdit.machineLearning.transcription.enabled}
            disabled={disabled || !configToEdit.machineLearning.enabled}
          />
          ...
          <SettingSelect
            ...
            desc={$t('admin.machine_learning_transcription_language_description')}
            name="transcription-language"
            value={configToEdit.machineLearning.transcription.language ?? AUTO_LANGUAGE}
            options={languageOptions}
            onSelect={(setting) => {
              const value = String(setting);
              configToEdit.machineLearning.transcription.language = value === AUTO_LANGUAGE ? null : value;
            }}
            disabled={disabled ||
              !configToEdit.machineLearning.enabled ||
              !configToEdit.machineLearning.transcription.enabled}
            isEdited={configToEdit.machineLearning.transcription.language !==
              config.machineLearning.transcription.language}
          />

          <SettingInputField
            inputType={SettingInputFieldType.NUMBER}
            label={$t('admin.machine_learning_transcription_max_duration')}
            description={$t('admin.machine_learning_transcription_max_duration_description')}
            bind:value={configToEdit.machineLearning.transcription.maxDuration}
            min={1}
            disabled={disabled ||
              !configToEdit.machineLearning.enabled ||
              !configToEdit.machineLearning.transcription.enabled}
            isEdited={configToEdit.machineLearning.transcription.maxDuration !==
              config.machineLearning.transcription.maxDuration}
          />
        </div>
      </SettingAccordion>
